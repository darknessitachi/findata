...
public class TestStrategy implements Strategy {
	...
	public void onDepthUpdate(DateTime now, Depth depth, MarketCondition mc, TradeBlotter blotter, Broker broker) {
		...
		if (!executed) {
			...
			HexinOrder order1, order2;

			order1 = new HexinOrder(stockRepo.findOneByCode("600000"), 1000, 10, HexinOrder.HexinType.SIMPLE_BUY); // zhongxin
			order2 = new HexinOrder(stockRepo.findOneByCode("600048"), 1000, 1000, HexinOrder.HexinType.SIMPLE_SELL); // zhongxin
			orders.add(order1);
			orders.add(order2);
			...
			order1.addTag(HexinOrder.ORDER_TAG_BROKER, "IB");
			order2.addTag(HexinOrder.ORDER_TAG_BROKER, "IB");
			orders.add(order1);
			orders.add(order2);
			broker.sendOrder(orders);
			orders.stream().filter(o -> broker instanceof michael.findata.algoquant.execution.component.broker.Broker).forEach(o -> (
			...
			order1.addTag(HexinOrder.ORDER_TAG_BROKER, "Huatai");
			order2.addTag(HexinOrder.ORDER_TAG_BROKER, "Huatai");
			orders.add(order1);
			orders.add(order2);
			broker.sendOrder(orders);
			orders.stream().filter(o -> broker instanceof michael.findata.algoquant.execution.component.broker.Broker).forEach(o -> (
			...
			order1 = new HexinOrder(stockRepo.findOneByCode("600016"), 6000, 1, HexinOrder.HexinType.SIMPLE_BUY); // zhongxin
			order2 = new HexinOrder(stockRepo.findOneByCode("600104"), 1000, 100, HexinOrder.HexinType.SIMPLE_SELL); // zhongxin
			order1.addTag(HexinOrder.ORDER_TAG_BROKER, "Unknown");
			order2.addTag(HexinOrder.ORDER_TAG_BROKER, "Unknown");
			orders.add(order1);
			orders.add(order2);
			broker.sendOrder(orders);
			orders.stream().filter(o -> broker instanceof michael.findata.algoquant.execution.component.broker.Broker).forEach(o -> (
			...
			order1.addTag(HexinOrder.ORDER_TAG_BROKER, "to-be-determined");
			order2.addTag(HexinOrder.ORDER_TAG_BROKER, "to-be-determined");
			orders.add(order1);
			orders.add(order2);
			broker.sendOrder(orders);
			orders.stream().filter(o -> broker instanceof michael.findata.algoquant.execution.component.broker.Broker).forEach(o -> (
			...
			order1.addTag(HexinOrder.ORDER_TAG_BROKER, "Zhongxin");
			order2.addTag(HexinOrder.ORDER_TAG_BROKER, "Zhongxin");
			orders.add(order1);
			orders.add(order2);
			broker.sendOrder(orders);
			orders.stream().filter(o -> broker instanceof michael.findata.algoquant.execution.component.broker.Broker).forEach(o -> (
			...
			((MetaBroker)broker).printOrderListeners();
			LOGGER.info("Listener printed!!!");
		}
		executed = true;
	}
	...
	public void onExecution(DateTime now, Execution execution, MarketCondition mc, TradeBlotter blotter, Broker broker) {
		LOGGER.info("Order updated: {} from broker {}", execution.order(), broker);
	}
